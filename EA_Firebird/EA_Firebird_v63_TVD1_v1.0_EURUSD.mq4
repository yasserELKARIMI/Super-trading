...
// Renato added [2006 03 19 (Mar 19)]
//   RVI to reduce the DD in case of a runaway trend
//----------------------- USER INPUT
...
// Renato added
extern int DVLimit = 7; // included by Renato
...
string Name_Expert = "Firebird v63G"; // included by Renato
...
extern int MAXTrades=999; // maximum number of trades to have open at once
...
//Ron Adds
int Divergence=diverge(0,Fast_Period,Slow_Period,Fast_Price,Slow_Price,0)/Point;
...
double Slope = Divergence-diverge(0,Fast_Period,Slow_Period,Fast_Price,Slow_Price,1)/Point; // included by Renato

Comment("maxDD=",maxDD," - maxOO=",maxOO," - CurDiv=",Divergence);
...
if ( MyOrdersTotal() >= MAXTrades ) Comment("Orders limit!"); else {
...
//   if((myMA*(1+Percent/100))<Bid) Print(" Top, Bid ",myMA*(1+Percent/100),"  ",Bid);
   ...
   if ( (myMA*(1+Percent/100))<Bid && Direction!=22 && (Bid>=(LastPrice+(CurrentPipStep*Point)) || MyOrdersTotal()==0) && RVI<0 && Divergence<-DVLimit && Slope>0 ) // modified by Renato
 	  {
   ...
   if ( (myMA*(1-Percent/100))>Ask && Direction!=11 && (Ask<=(LastPrice-(CurrentPipStep*Point)) || MyOrdersTotal()==0) && RVI>0 && Divergence>DVLimit && Slope<0 ) // Go LONG -> Only buy if >= 30 pips below previous position entry // modified by Renato
     {
...
} // end of timefilter // included by Renato
} // end of MAXTrades limit // included by Renato
} // end of flag test                  
...
double diverge(int timeframe, int F_Period, int S_Period, int F_Price, int S_Price, int mypos)
  {
   int i;
   double maF1, maF2, maS1, maS2;
   double dv1, dv2;
   maF1=iMA(Symbol(),timeframe,F_Period,0,MODE_SMA,F_Price,mypos);
   maS1=iMA(Symbol(),timeframe,S_Period,0,MODE_SMA,S_Price,mypos);
   dv1=(maF1-maS1);
   return(dv1);
  }
