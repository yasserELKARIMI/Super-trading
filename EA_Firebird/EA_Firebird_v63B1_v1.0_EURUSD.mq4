...
extern int Stoploss = 500;// total loss on all open positions in pips
...
int MagicNumber; // modified by Renato 
...
if (Day()!=5 || TradeOnFriday >0)
{
   int cnt=0, total;
   int myTotal =0;                     // hdb
   total=OrdersTotal(); // modified by Renato
...
//   if((myMA*(1+Percent/100))<Bid) Print(" Top, Bid ",myMA*(1+Percent/100),"  ",Bid);
   ...
   if((myMA*(1+Percent/100))<Bid && Direction!=22 && (Bid>=(LastPrice+(CurrentPipStep*Point))||total==0)) // Go SHORT -> Only sell if >= 30 pips above previous position entry	
 	  {
      OrderSend(Symbol(),OP_SELL,Lots,Bid,slip,Bid+(Stoploss*Point),Bid-(TakeProfit*Point),Name_Expert,MagicNumber,0,Red);  // modified by Renato
      OrderWatcher=1;
      Direction=2;
     }   
   if((myMA*(1-Percent/100))>Ask && Direction!=11 && (Ask<=(LastPrice-(CurrentPipStep*Point))||total==0)) // Go LONG -> Only buy if >= 30 pips below previous position entry	 
     {
      OrderSend(Symbol(),OP_BUY,Lots,Ask,slip,Ask-(Stoploss*Point),Ask+(TakeProfit*Point),Name_Expert,MagicNumber,0,Blue);  // modified by Renato
      OrderWatcher=1;
      Direction=1;
     } 
   }     
...
int MagicfromSymbol() { // included by Renato 
   int magic=0;  
   for (int i=0; i<5; i++) {  
      magic=magic*3+StringGetChar(Symbol(),i);  
   }  
...
int MyOrdersTotal() { // included by Renato
   ...
   for (int i=0; i<OrdersTotal(); i++) { 
     if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) 
       if ( (OrderSymbol()==Symbol()) && (OrderMagicNumber()==MagicNumber) && (OrderComment()==Name_Expert) ) total++; 
   }  
   return(total); 
} 
